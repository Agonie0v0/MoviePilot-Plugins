...
class autocleanunlinkedseed(_PluginBase):
    ...
    # 插件配置项ID前缀
    plugin_config_prefix = "autocleanunlinkedseed"
    # 加载顺序
    plugin_order = 1
    # 可使用的用户级别
    auth_level = 1
    ...
    def check_is_unlinked(self, path_str: str) -> bool:
        ...
        # 注意：Linux 系统下目录本身不支持硬链接逻辑（仅以视频文件为准）
        if path.is_file():
